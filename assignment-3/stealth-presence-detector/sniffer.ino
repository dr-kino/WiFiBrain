...
void setup() { 
  ...
  pinMode(D6, OUTPUT);
  pinMode(D7, OUTPUT);
  pinMode(D4, OUTPUT); // (RC) Adapted
  digitalWrite(D4, HIGH); // (RC) Adapted
  Serial.begin(115200);
  esppl_init(cb);
}
...
void red() { digitalWrite(D5, HIGH); }  // Turn ON the red LED
void blue() { digitalWrite(D7, HIGH); } // Turn ON the blue LED
void green() { digitalWrite(D6, HIGH); } // Turn ON the green LED
...
void cb(esppl_frame_info *info) {
  for (int i=0; i<LIST_SIZE; i++) {
    if (maccmp(info->sourceaddr, friendmac[i]) || maccmp(info->receiveraddr, friendmac[i])) {
      ...
      digitalWrite(D4, LOW); // (RC) Adapted
